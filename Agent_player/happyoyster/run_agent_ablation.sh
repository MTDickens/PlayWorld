...
WORLDPLAY_DIR="${HAPPYOYSTER_DATA_ROOT:-${REPO_ROOT}/data}"
...
case "$AGENT_LABEL" in
  ...
  gemini31pro)
    ...
    export AGENT_MODEL="${AGENT_MODEL:-gemini-3.1-pro}"
    ;;
...
OUT_ROOT="${HAPPYOYSTER_ABLATION_OUT_ROOT:-${REPO_ROOT}/outputs/happyoyster-ablation}"
...
export HAPPYOYSTER_FLAT_OUTPUTS="${HAPPYOYSTER_FLAT_OUTPUTS:-0}"
export HAPPYOYSTER_DATA_FILES="${HAPPYOYSTER_DATA_FILES:-GC:gc/data.json,IF:if/data.json,OE:insight/data.json,OE:outsight/data.json}"
...
python3 "${SCRIPT_DIR}/player.py" 2>&1 | tee "$LOG_PATH"
